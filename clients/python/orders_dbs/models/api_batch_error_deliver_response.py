...
class ApiBatchErrorDeliverResponse(BaseModel):
    ...
    detail: Optional[StrictStr] = Field(default=None, description="- `NotFound` — сборочное задание не найдено - `StatusMismatch` — операция невозможна для этого статуса сборочного задания - `ImeiIsNotFilled` — не заполнен IMEI - `MetaValidationFail` — ошибки валидации метаданных ", json_schema_extra={"examples": ["MetaValidationFail"]})
    meta_details: Optional[List[ApiBatchErrorDeliverResponseMetaDetailsInner]] = Field(default=None, description="Детали ошибки валидации метаданных", alias="metaDetails")
