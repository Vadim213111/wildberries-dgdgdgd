...
ADVV1STATSPOSTREQUESTINNER_ONE_OF_SCHEMAS = ["RequestWithCampaignID", "RequestWithDate", "RequestWithInterval"]

class AdvV1StatsPostRequestInner(BaseModel):
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = AdvV1StatsPostRequestInner.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AdvV1StatsPostRequestInner with oneOf schemas: RequestWithCampaignID, RequestWithDate, RequestWithInterval. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in AdvV1StatsPostRequestInner with oneOf schemas: RequestWithCampaignID, RequestWithDate, RequestWithInterval. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into AdvV1StatsPostRequestInner with oneOf schemas: RequestWithCampaignID, RequestWithDate, RequestWithInterval. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AdvV1StatsPostRequestInner with oneOf schemas: RequestWithCampaignID, RequestWithDate, RequestWithInterval. Details: " + ", ".join(error_messages))
        else:
            return instance
