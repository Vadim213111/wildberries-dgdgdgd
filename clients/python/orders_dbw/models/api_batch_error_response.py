...
class ApiBatchErrorResponse(BaseModel):
    ...
    """ # noqa: E501
    code: StrictInt = Field(description="Код ошибки:   - `404`   - `409` ", json_schema_extra={"examples": [404]})
    detail: StrictStr = Field(description="- `NotFound` — сборочное задание не найдено - `StatusMismatch` — операция невозможна для этого статуса сборочного задания - `ImeiIsNotFilled` — не заполнен IMEI - `MetaValidationFail` — ошибки валидации метаданных ", json_schema_extra={"examples": ["NotFound"]})
    meta_details: Optional[List[ApiBatchErrorResponseMetaDetailsInner]] = Field(default=None, description="Детали ошибки валидации метаданных", alias="metaDetails")
