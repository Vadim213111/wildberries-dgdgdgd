/* tslint:disable */
/* eslint-disable */
/**
 * Маркетинг и продвижение
 * <div class=\"description_important\">     Узнать больше о маркетинге и продвижении можно в <a href=\"https://seller.wildberries.ru/instructions/category/59d92bd3-6ea0-40f2-b762-ca8835d7d42e?goBackOption=prevRoute&categoryId=479385c6-de01-4b4d-ad4e-ed941e65582e\">справочном центре</a> </div>  Методы маркетинга и продвижения позволяют:   1. Получать информацию о кампаниях [продвижения](/openapi/promotion#tag/Kampanii) и [медиакампаниях](/openapi/promotion#tag/Media).   2. [Создавать](/openapi/promotion#tag/Sozdanie-kampanij) и [управлять](/openapi/promotion#tag/Upravlenie-kampaniyami) кампаниями.   3. Управлять [финансами](/openapi/promotion#tag/Finansy) кампаний.   4. Выгружать [статистику](/openapi/promotion#tag/Statistika) кампаний продвижения и медиакампаний.   5. Работать с [календарём акций](/openapi/promotion#tag/Kalendar-akcij).  Данные синхронизируются с базой раз в 3 минуты. Статусы кампаний меняются раз в минуту. Ставки кампаний меняются раз в 30 секунд. 
 *
 * The version of the OpenAPI document: promotion
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface AdvV1BudgetDepositPost400Response
 */
export interface AdvV1BudgetDepositPost400Response {
    /**
     * 
     * @type {string}
     * @memberof AdvV1BudgetDepositPost400Response
     */
    error?: string;
}

/**
 * Check if a given object implements the AdvV1BudgetDepositPost400Response interface.
 */
export function instanceOfAdvV1BudgetDepositPost400Response(value: object): value is AdvV1BudgetDepositPost400Response {
    return true;
}

export function AdvV1BudgetDepositPost400ResponseFromJSON(json: any): AdvV1BudgetDepositPost400Response {
    return AdvV1BudgetDepositPost400ResponseFromJSONTyped(json, false);
}

export function AdvV1BudgetDepositPost400ResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): AdvV1BudgetDepositPost400Response {
    if (json == null) {
        return json;
    }
    return {
        
        'error': json['error'] == null ? undefined : json['error'],
    };
}

export function AdvV1BudgetDepositPost400ResponseToJSON(json: any): AdvV1BudgetDepositPost400Response {
    return AdvV1BudgetDepositPost400ResponseToJSONTyped(json, false);
}

export function AdvV1BudgetDepositPost400ResponseToJSONTyped(value?: AdvV1BudgetDepositPost400Response | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'error': value['error'],
    };
}

