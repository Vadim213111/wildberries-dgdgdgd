/* tslint:disable */
/* eslint-disable */
/**
 * Маркетинг и продвижение
 * <div class=\"description_important\">     Узнать больше о маркетинге и продвижении можно в <a href=\"https://seller.wildberries.ru/instructions/category/59d92bd3-6ea0-40f2-b762-ca8835d7d42e?goBackOption=prevRoute&categoryId=479385c6-de01-4b4d-ad4e-ed941e65582e\">справочном центре</a> </div>  Методы маркетинга и продвижения позволяют:   1. Получать информацию о кампаниях [продвижения](/openapi/promotion#tag/Kampanii) и [медиакампаниях](/openapi/promotion#tag/Media).   2. [Создавать](/openapi/promotion#tag/Sozdanie-kampanij) и [управлять](/openapi/promotion#tag/Upravlenie-kampaniyami) кампаниями.   3. Управлять [финансами](/openapi/promotion#tag/Finansy) кампаний.   4. Выгружать [статистику](/openapi/promotion#tag/Statistika) кампаний продвижения и медиакампаний.   5. Работать с [календарём акций](/openapi/promotion#tag/Kalendar-akcij).  Данные синхронизируются с базой раз в 3 минуты. Статусы кампаний меняются раз в минуту. Ставки кампаний меняются раз в 30 секунд. 
 *
 * The version of the OpenAPI document: promotion
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface AdvV2SupplierNmsPost200ResponseInner
 */
export interface AdvV2SupplierNmsPost200ResponseInner {
    /**
     * Название товара
     * @type {string}
     * @memberof AdvV2SupplierNmsPost200ResponseInner
     */
    title?: string;
    /**
     * Артикул WB
     * @type {number}
     * @memberof AdvV2SupplierNmsPost200ResponseInner
     */
    nm?: number;
    /**
     * ID предмета
     * @type {number}
     * @memberof AdvV2SupplierNmsPost200ResponseInner
     */
    subjectId?: number;
}

/**
 * Check if a given object implements the AdvV2SupplierNmsPost200ResponseInner interface.
 */
export function instanceOfAdvV2SupplierNmsPost200ResponseInner(value: object): value is AdvV2SupplierNmsPost200ResponseInner {
    return true;
}

export function AdvV2SupplierNmsPost200ResponseInnerFromJSON(json: any): AdvV2SupplierNmsPost200ResponseInner {
    return AdvV2SupplierNmsPost200ResponseInnerFromJSONTyped(json, false);
}

export function AdvV2SupplierNmsPost200ResponseInnerFromJSONTyped(json: any, ignoreDiscriminator: boolean): AdvV2SupplierNmsPost200ResponseInner {
    if (json == null) {
        return json;
    }
    return {
        
        'title': json['title'] == null ? undefined : json['title'],
        'nm': json['nm'] == null ? undefined : json['nm'],
        'subjectId': json['subjectId'] == null ? undefined : json['subjectId'],
    };
}

export function AdvV2SupplierNmsPost200ResponseInnerToJSON(json: any): AdvV2SupplierNmsPost200ResponseInner {
    return AdvV2SupplierNmsPost200ResponseInnerToJSONTyped(json, false);
}

export function AdvV2SupplierNmsPost200ResponseInnerToJSONTyped(value?: AdvV2SupplierNmsPost200ResponseInner | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'title': value['title'],
        'nm': value['nm'],
        'subjectId': value['subjectId'],
    };
}

