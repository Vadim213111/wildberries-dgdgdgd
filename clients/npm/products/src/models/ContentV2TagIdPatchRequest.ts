/* tslint:disable */
/* eslint-disable */
/**
 * Работа с товарами
 * С помощью методов этого раздела вы можете:   - [создавать](/openapi/work-with-products#tag/Sozdanie-kartochek-tovarov) и [редактировать](/openapi/work-with-products#tag/Kartochki-tovarov) карточки товаров   - получать [категории, предметы, характеристики и бренды товаров](/openapi/work-with-products#tag/Kategorii-predmety-i-harakteristiki)   - загружать [медиафайлы](/openapi/work-with-products#tag/Mediafajly) в карточки товаров   - настраивать [ярлыки](/openapi/work-with-products#tag/Yarlyki) для поиска товаров   - устанавливать [цены и скидки](/openapi/work-with-products#tag/Ceny-i-skidki)   - управлять [остатками товаров](/openapi/work-with-products#tag/Ostatki-na-skladah-prodavca) и [складами](/openapi/work-with-products#tag/Sklady-prodavca), если вы работаете по модели продаж со склада продавца    <div class=\"description_ref\">     Узнать, как использовать методы в бизнес-кейсах, можно в <a href=\"https://dev.wildberries.ru/knowledge-base/articles/019d49a4-1320-71bb-9dac-8ba07e7177ce/rabota-s-tovarami\">инструкции</a> по <strong>работе с товарами</strong>   </div> 
 *
 * The version of the OpenAPI document: products
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface ContentV2TagIdPatchRequest
 */
export interface ContentV2TagIdPatchRequest {
    /**
     * Цвет ярлыка
     * @type {string}
     * @memberof ContentV2TagIdPatchRequest
     */
    color?: string;
    /**
     * Имя ярлыка
     * @type {string}
     * @memberof ContentV2TagIdPatchRequest
     */
    name?: string;
}

/**
 * Check if a given object implements the ContentV2TagIdPatchRequest interface.
 */
export function instanceOfContentV2TagIdPatchRequest(value: object): value is ContentV2TagIdPatchRequest {
    return true;
}

export function ContentV2TagIdPatchRequestFromJSON(json: any): ContentV2TagIdPatchRequest {
    return ContentV2TagIdPatchRequestFromJSONTyped(json, false);
}

export function ContentV2TagIdPatchRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): ContentV2TagIdPatchRequest {
    if (json == null) {
        return json;
    }
    return {
        
        'color': json['color'] == null ? undefined : json['color'],
        'name': json['name'] == null ? undefined : json['name'],
    };
}

export function ContentV2TagIdPatchRequestToJSON(json: any): ContentV2TagIdPatchRequest {
    return ContentV2TagIdPatchRequestToJSONTyped(json, false);
}

export function ContentV2TagIdPatchRequestToJSONTyped(value?: ContentV2TagIdPatchRequest | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'color': value['color'],
        'name': value['name'],
    };
}

