/* tslint:disable */
/* eslint-disable */
/**
 * Работа с товарами
 * С помощью методов этого раздела вы можете:   - [создавать](/openapi/work-with-products#tag/Sozdanie-kartochek-tovarov) и [редактировать](/openapi/work-with-products#tag/Kartochki-tovarov) карточки товаров   - получать [категории, предметы, характеристики и бренды товаров](/openapi/work-with-products#tag/Kategorii-predmety-i-harakteristiki)   - загружать [медиафайлы](/openapi/work-with-products#tag/Mediafajly) в карточки товаров   - настраивать [ярлыки](/openapi/work-with-products#tag/Yarlyki) для поиска товаров   - устанавливать [цены и скидки](/openapi/work-with-products#tag/Ceny-i-skidki)   - управлять [остатками товаров](/openapi/work-with-products#tag/Ostatki-na-skladah-prodavca) и [складами](/openapi/work-with-products#tag/Sklady-prodavca), если вы работаете по модели продаж со склада продавца    <div class=\"description_ref\">     Узнать, как использовать методы в бизнес-кейсах, можно в <a href=\"https://dev.wildberries.ru/knowledge-base/articles/019d49a4-1320-71bb-9dac-8ba07e7177ce/rabota-s-tovarami\">инструкции</a> по <strong>работе с товарами</strong>   </div> 
 *
 * The version of the OpenAPI document: products
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { GoodsListSizesInner } from './GoodsListSizesInner';
import {
    GoodsListSizesInnerFromJSON,
    GoodsListSizesInnerFromJSONTyped,
    GoodsListSizesInnerToJSON,
    GoodsListSizesInnerToJSONTyped,
} from './GoodsListSizesInner';

/**
 * Размеры товара
 * @export
 * @interface GoodsList
 */
export interface GoodsList {
    /**
     * Артикул WB
     * @type {number}
     * @memberof GoodsList
     */
    nmID?: number;
    /**
     * Артикул продавца
     * @type {string}
     * @memberof GoodsList
     */
    vendorCode?: string;
    /**
     * Размер
     * @type {Array<GoodsListSizesInner>}
     * @memberof GoodsList
     */
    sizes?: Array<GoodsListSizesInner>;
    /**
     * Валюта, по стандарту ISO 4217
     * @type {string}
     * @memberof GoodsList
     */
    currencyIsoCode4217?: string;
    /**
     * Скидка, %
     * @type {number}
     * @memberof GoodsList
     */
    discount?: number;
    /**
     * Скидка WB Клуба, %
     * @type {number}
     * @memberof GoodsList
     */
    clubDiscount?: number;
    /**
     * Можно ли устанавливать цены отдельно для разных размеров (зависит от категории товара):
     *   - `true` — можно
     *   - `false` — нельзя
     * 
     * @type {boolean}
     * @memberof GoodsList
     */
    editableSizePrice?: boolean;
    /**
     * Признак неликвидного товара:
     *   - `true` — неликвидный товар с [низким индексом остатка](https://seller.wildberries.ru/instructions/ru/ru/material/stocks-index?categoryId=e324ce0f-9a2a-4b8d-8fd1-72f751b09b3b&goBackOption=prevRoute#%D1%83%D1%80%D0%BE%D0%B2%D0%BD%D0%B8-%D0%B8%D0%BD%D0%B4%D0%B5%D0%BA%D1%81%D0%B0-%D0%BE%D1%81%D1%82%D0%B0%D1%82%D0%BA%D0%B0)
     *   - Поле отсутствует — ликвидный товар
     * 
     * @type {boolean}
     * @memberof GoodsList
     */
    isBadTurnover?: boolean;
}

/**
 * Check if a given object implements the GoodsList interface.
 */
export function instanceOfGoodsList(value: object): value is GoodsList {
    return true;
}

export function GoodsListFromJSON(json: any): GoodsList {
    return GoodsListFromJSONTyped(json, false);
}

export function GoodsListFromJSONTyped(json: any, ignoreDiscriminator: boolean): GoodsList {
    if (json == null) {
        return json;
    }
    return {
        
        'nmID': json['nmID'] == null ? undefined : json['nmID'],
        'vendorCode': json['vendorCode'] == null ? undefined : json['vendorCode'],
        'sizes': json['sizes'] == null ? undefined : ((json['sizes'] as Array<any>).map(GoodsListSizesInnerFromJSON)),
        'currencyIsoCode4217': json['currencyIsoCode4217'] == null ? undefined : json['currencyIsoCode4217'],
        'discount': json['discount'] == null ? undefined : json['discount'],
        'clubDiscount': json['clubDiscount'] == null ? undefined : json['clubDiscount'],
        'editableSizePrice': json['editableSizePrice'] == null ? undefined : json['editableSizePrice'],
        'isBadTurnover': json['isBadTurnover'] == null ? undefined : json['isBadTurnover'],
    };
}

export function GoodsListToJSON(json: any): GoodsList {
    return GoodsListToJSONTyped(json, false);
}

export function GoodsListToJSONTyped(value?: GoodsList | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'nmID': value['nmID'],
        'vendorCode': value['vendorCode'],
        'sizes': value['sizes'] == null ? undefined : ((value['sizes'] as Array<any>).map(GoodsListSizesInnerToJSON)),
        'currencyIsoCode4217': value['currencyIsoCode4217'],
        'discount': value['discount'],
        'clubDiscount': value['clubDiscount'],
        'editableSizePrice': value['editableSizePrice'],
        'isBadTurnover': value['isBadTurnover'],
    };
}

