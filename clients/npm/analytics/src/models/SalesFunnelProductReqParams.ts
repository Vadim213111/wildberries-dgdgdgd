/* tslint:disable */
/* eslint-disable */
/**
 * Аналитика и данные
 * <div class=\"description_important\">   Узнать больше об аналитике и данных можно в <a href=\"https://seller.wildberries.ru/instructions/ru/ru/subcategory/seller-analytics\">справочном центре</a> </div>  В разделе описаны методы получения:   1. [Воронки продаж](/openapi/analytics#tag/Voronka-prodazh)   2. [Поисковых запросов по вашим товарам](/openapi/analytics#tag/Poiskovye-zaprosy-po-vashim-tovaram)   3. [Истории остатков](/openapi/analytics#tag/Istoriya-ostatkov)   4. [Аналитики продавца в формате CSV](/openapi/analytics#tag/Analitika-prodavca-CSV) 
 *
 * The version of the OpenAPI document: analytics
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * Параметры отчёта
 * @export
 * @interface SalesFunnelProductReqParams
 */
export interface SalesFunnelProductReqParams {
    /**
     * Артикулы WB, по которым составить отчёт. Оставьте пустым, чтобы получить отчёт обо всех товарах
     * 
     * @type {Array<number>}
     * @memberof SalesFunnelProductReqParams
     */
    nmIDs?: Array<number>;
    /**
     * Список ID предметов для фильтрации
     * @type {Array<number>}
     * @memberof SalesFunnelProductReqParams
     */
    subjectIds?: Array<number>;
    /**
     * Список брендов для фильтрации
     * @type {Array<string>}
     * @memberof SalesFunnelProductReqParams
     */
    brandNames?: Array<string>;
    /**
     * Список ID ярлыков для фильтрации
     * @type {Array<number>}
     * @memberof SalesFunnelProductReqParams
     */
    tagIds?: Array<number>;
    /**
     * Начало периода
     * @type {Date}
     * @memberof SalesFunnelProductReqParams
     */
    startDate: Date;
    /**
     * Конец периода
     * @type {Date}
     * @memberof SalesFunnelProductReqParams
     */
    endDate: Date;
    /**
     * Временная зона, по умолчанию Europe/Moscow
     * 
     * @type {string}
     * @memberof SalesFunnelProductReqParams
     */
    timezone?: string;
    /**
     * Как сгруппировать данные (по умолчанию по дням):
     * 
     *   * `day` — по дням
     *   * `week` — по неделям
     *   * `month` — по месяцам
     * 
     * @type {SalesFunnelProductReqParamsAggregationLevelEnum}
     * @memberof SalesFunnelProductReqParams
     */
    aggregationLevel?: SalesFunnelProductReqParamsAggregationLevelEnum;
    /**
     * Скрыть удалённые карточки товаров
     * @type {boolean}
     * @memberof SalesFunnelProductReqParams
     */
    skipDeletedNm?: boolean;
}


/**
 * @export
 */
export const SalesFunnelProductReqParamsAggregationLevelEnum = {
    Day: 'day',
    Week: 'week',
    Month: 'month'
} as const;
export type SalesFunnelProductReqParamsAggregationLevelEnum = typeof SalesFunnelProductReqParamsAggregationLevelEnum[keyof typeof SalesFunnelProductReqParamsAggregationLevelEnum];


/**
 * Check if a given object implements the SalesFunnelProductReqParams interface.
 */
export function instanceOfSalesFunnelProductReqParams(value: object): value is SalesFunnelProductReqParams {
    if (!('startDate' in value) || value['startDate'] === undefined) return false;
    if (!('endDate' in value) || value['endDate'] === undefined) return false;
    return true;
}

export function SalesFunnelProductReqParamsFromJSON(json: any): SalesFunnelProductReqParams {
    return SalesFunnelProductReqParamsFromJSONTyped(json, false);
}

export function SalesFunnelProductReqParamsFromJSONTyped(json: any, ignoreDiscriminator: boolean): SalesFunnelProductReqParams {
    if (json == null) {
        return json;
    }
    return {
        
        'nmIDs': json['nmIDs'] == null ? undefined : json['nmIDs'],
        'subjectIds': json['subjectIds'] == null ? undefined : json['subjectIds'],
        'brandNames': json['brandNames'] == null ? undefined : json['brandNames'],
        'tagIds': json['tagIds'] == null ? undefined : json['tagIds'],
        'startDate': (new Date(json['startDate'])),
        'endDate': (new Date(json['endDate'])),
        'timezone': json['timezone'] == null ? undefined : json['timezone'],
        'aggregationLevel': json['aggregationLevel'] == null ? undefined : json['aggregationLevel'],
        'skipDeletedNm': json['skipDeletedNm'] == null ? undefined : json['skipDeletedNm'],
    };
}

export function SalesFunnelProductReqParamsToJSON(json: any): SalesFunnelProductReqParams {
    return SalesFunnelProductReqParamsToJSONTyped(json, false);
}

export function SalesFunnelProductReqParamsToJSONTyped(value?: SalesFunnelProductReqParams | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'nmIDs': value['nmIDs'],
        'subjectIds': value['subjectIds'],
        'brandNames': value['brandNames'],
        'tagIds': value['tagIds'],
        'startDate': value['startDate'].toISOString().substring(0,10),
        'endDate': value['endDate'].toISOString().substring(0,10),
        'timezone': value['timezone'],
        'aggregationLevel': value['aggregationLevel'],
        'skipDeletedNm': value['skipDeletedNm'],
    };
}

