...
class SalesReportListRes(BaseModel):
    ...
    delivery_service_sum: StrictStr = Field(description="Стоимость логистики", alias="deliveryServiceSum")
    ...
    deduction_sum: StrictStr = Field(description="Прочие удержания и выплаты", alias="deductionSum")
    penalty_sum: StrictStr = Field(description="Общая сумма штрафов", alias="penaltySum")
    additional_payment_sum: StrictStr = Field(description="Корректировка Вознаграждения Вайлдберриз (ВВ)", alias="additionalPaymentSum")
    cashback_amount_sum: StrictStr = Field(description="Сумма, удержанная за начисленные баллы программы лояльности", alias="cashbackAmountSum")
