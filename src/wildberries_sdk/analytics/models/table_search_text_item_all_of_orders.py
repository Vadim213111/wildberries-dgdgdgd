...
class TableSearchTextItemAllOfOrders(BaseModel):
    ...
    percentile: StrictInt = Field(description="Процент, на который показатель заказов выше, чем у карточек конкурентов по поисковому запросу", json_schema_extra={"examples": [50]})
